...
class UnpairedDataset(Dataset):
    ...
    def __init__(
        ...
        for path in self.A_paths:
            category = self.mode #path.split('_')[0]
    ...
    def _load_and_resize_img(self, image_path, image_size, resize_mode='interpolate'):
        ...
        if resize_mode == 'interpolate':
            newSpace = [osz * osp / nsz for osz, osp, nsz in zip(originSize, image.GetSpacing(), newSize)]
    ...
    def __getitem__(self, index):
        ...
        return {'A_img': A_img, 'B_img': B_img, 'A_path': A_img_path, 'B_path': B_img_path, 'paired_flag': self.is_paired, 'label': label, 'img_name': str(img_name)}
