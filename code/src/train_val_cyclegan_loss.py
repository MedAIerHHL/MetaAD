...
def add_noise_to_subregion(image, noise_type, noise_scale, sampling_rate, intensity=1, mean=0, std=0.2):
    ...
    return image
def add_noise_to_batch(batch, noise_type, noise_scale, samplinoise_scaleng_rate, intensity=1, mean=0, std=1):
    ...
    with torch.no_grad():
        for _, data in enumerate(val_loader):
            ...
            if is_paired:
                ...
                recon_loss_FDG = F.l1_loss(syn_FDG_img, real_FDG_img)
                recon_loss_CFT = F.l1_loss(syn_CFT_img, real_CFT_img)
            ...
            identity_loss_FDG = F.l1_loss(same_FDG_img, real_FDG_img)
            identity_loss_CFT = F.l1_loss(same_CFT_img, real_CFT_img)
        ...
        if cfg.TEST.mask == True:
            mask_FDG_img = gen_random_mask(real_FDG_img, cfg.MODEL.mask_k, cfg.MODEL.mask_size, cfg.MODEL.mask_prob)
        ...
        show_save_img(error_list, error_filename, os.path.join(exexperiment_dir, 'samples_viz'), vmin=-1, vmax=1, colormap='bwr')
        logging.info('\n')
...
def train_cyclegan_loss(cfg, args):
    ...
    for test_data_file in cfg.TEST.test_data_file:
        # Define test dataset
        test_data = PETDataset(
            cfg.GLOBAL.data_dir,
            test_data_file, # cfg.TEST.test_data_file,
            cfg.TEST.image_size,
            cfg.TEST.data_norm,
        )
        ...
        test_sample_iterator.append(sample_iterator_test)
    ...
    for iter_index in tqdm(range(start_iter, cfg.TRAIN.max_iter), desc='Training'):
        ...
        # (6) compute contrast loss
        # contrast_loss_FDG = contrast_loss(noise_recon_FDG_img.view(noise_recon_FDG_img.shape[0], -1),
        #                                   noise_real_FDG_img.view(noise_real_FDG_img.shape[0], -1),
        #                                   -torch.ones(noise_recon_FDG_img.shape[0]).to('cuda')) \
        #                     * cfg.LOSS.contrast_weight
        # contrast_loss_CFT = contrast_loss(noise_recon_CFT_img.view(noise_recon_CFT_img.shape[0], -1),
        #                                   noise_real_CFT_img.view(noise_real_CFT_img.shape[0], -1),
        #                                   -torch.ones(noise_recon_CFT_img.shape[0]).to('cuda')) \
        #                     * cfg.LOSS.contrast_weight
        contrast_loss_FDG = contrast_loss(noise_recon_FDG_img, noise_real_FDG_img) *cfg.LOSS.contrast_weight
        contrast_loss_CFT = contrast_loss(noise_recon_CFT_img, noise_real_CFT_img) * cfg.LOSS.contrast_weight
        contrast_loss_all = contrast_loss_FDG + contrast_loss_CFT

        # backward and optimize
        total_loss_G = mid_contrast_loss + cycle_loss + GAN_loss_G + identity_loss + recon_loss + noise_cycle_loss + contrast_loss_all
        ...
        if cfg.TRAIN.log_interval and cur_iter % cfg.TRAIN.log_interval == 0:
            ...
            if paired_counts > 0:
                logging.info(
                    ...
                    .format(cur_iter, recon_loss_CFT.item()/cfg.LOSS.recon_weight, recon_loss_FDG.item()/cfg.LOSS.recon_weight)
                )
        ...
        if (cur_iter % cfg.TRAIN.val_interval == 0 and cur_iter >= cfg.TRAIN.val_interval) or cur_iter == cfg.TRAIN.max_iter:
            ...
            for i in range(len(test_data_list)):
                ...
                df['label'] = pd.Series(test_label_dict)
                for key in test_label_dict.keys():
                    df.loc[key, 'label'] = test_label_dict[key]
                    for key2 in test_scores_dict.keys():
                        df.loc[key, key2 + '_score'] = test_scores_dict[key2][key]
                    for key3 in test_index_dict.keys():
                        string_list = list(map(str, test_index_dict[key3][key]))
                        result = ', '.join(string_list)
                        df.loc[key, key3 + '_index'] = result
                df.to_csv(os.path.join(df_save_dir, 'label_scores_{}_{}.csv'.format(cur_iter, i)))
                """

                for key in test_scores_dict.keys():
                    perf_dict, thre = image_wise_anomaly_detection(cfg, test_scores_dict[key], test_label_dict)
                    all_test_scores_perf['test{}'.format(i)][key] = perf_dict
                    a = 'perf_'+key
            ...
            for key in list(all_test_scores_perf.values())[0].keys():
                model_type_auc = []
                model_type_acc = []
                model_type_sen = []
                model_type_spe = []
                model_type_f1 = []
                for test in all_test_scores_perf.values():
                    value = test[key]
                    model_type_auc.append(value['roc_auc'])
                    model_type_acc.append(value['acc'])
                    model_type_sen.append(value['sen'])
                    model_type_spe.append(value['spe'])
                    model_type_f1.append(value['f1'])
                model_type_auc_mean, model_type_auc_std = np.mean(model_type_auc), np.std(model_type_auc)
                model_type_acc_mean, model_type_acc_std = np.mean(model_type_acc), np.std(model_type_acc)
                model_type_sen_mean, model_type_sen_std = np.mean(model_type_sen), np.std(model_type_sen)
                model_type_spe_mean, model_type_spe_std = np.mean(model_type_spe), np.std(model_type_spe)
                model_type_f1_mean, model_type_f1_std = np.mean(model_type_f1), np.std(model_type_f1)

                logging.info(
                    '{}_test_{} Auc: {:.3f}±{:.3f}      Acc: {:.3f}±{:.3f}      Sen: {:.3f}±{:.3f}      Spe: {:.3f}±{:.3f}     F1: {:.3f}±{:.3f}'.format(
                        cur_iter,
                        key[0:6],
                        model_type_auc_mean,
                        model_type_auc_std,
